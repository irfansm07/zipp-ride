...
function HomeScreen({ bookingTab, setBookingTab, category, setCategory, pickup, setPickup, dropoff, setDropoff, womenShare, setWomenShare, maxShare, setMaxShare, onSearch, activeRide, etaCount, onSOS }) {
  ...
  const [requestStatus, setRequestStatus] = useState("No request yet.");
  const [passengerUpdate, setPassengerUpdate] = useState("No updates yet. Enter train number and station.");

  const trainStatusSamples = [
    { state:"On Time", delay:"0 min", nextStopIn:"14 min" },
    { state:"Approaching Station", delay:"3 min", nextStopIn:"6 min" },
    { state:"Slight Delay", delay:"8 min", nextStopIn:"18 min" },
  ];

  const handleNoDelaySearch = () => {
    const train = trainNumber.trim();
    const station = pickupStation.trim();
    if (!train || !station) {
      setPassengerUpdate("Please enter both train number and station.");
      return;
    }

    const pickedStatus = trainStatusSamples[Math.floor(Math.random() * trainStatusSamples.length)];
    setTrainStatus({ train, station, ...pickedStatus });
    setRequestStatus(`Request sent to nearby drivers in driver app for ${station}.`);
    setPassengerUpdate(`Searching nearby drivers for Train ${train} pickup at ${station}...`);

    setTimeout(() => {
      setRequestStatus("Driver assigned");
      setPassengerUpdate(`Driver assigned for Train ${train} at ${station}. Driver ETA to station: 7 min.`);
    }, 2200);
  };
  ...
  return (
    <div style={{animation:"fadeIn .4s ease"}}>
      ...
      {/* No Delay Cab - User App */}
      <div style={{margin:"16px 20px 0",background:"#12121a",border:"1px solid #2a2a3a",borderRadius:20,padding:16}}>
        <div style={{fontFamily:"Syne,sans-serif",fontSize:16,fontWeight:800,marginBottom:4}}>No Delay Cab</div>
        <div style={{fontSize:12,color:"#8888aa",marginBottom:12}}>
          User flow only: request is sent to driver app, and you get live updates here.
        </div>

        <div style={{display:"grid",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10,background:"#1c1c28",border:"1px solid #2a2a3a",borderRadius:10,padding:"11px 13px"}}>
            <span style={{fontSize:14}}>🚆</span>
            <input value={trainNumber} onChange={e => setTrainNumber(e.target.value)} placeholder="Train number (e.g. 12627)" />
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,background:"#1c1c28",border:"1px solid #2a2a3a",borderRadius:10,padding:"11px 13px"}}>
            <span style={{fontSize:14}}>📍</span>
            <input value={pickupStation} onChange={e => setPickupStation(e.target.value)} placeholder="Pickup station (e.g. Chennai Central)" />
          </div>
          <button onClick={handleNoDelaySearch} style={{background:"linear-gradient(135deg,#00e5a0,#00b87a)",color:"#080810",border:"none",borderRadius:12,padding:"12px",fontFamily:"Syne,sans-serif",fontSize:13,fontWeight:800}}>
            Search No Delay Cab
          </button>
        </div>

        <div style={{marginTop:12,background:"#1c1c28",border:"1px solid #2a2a3a",borderRadius:12,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:"#7b5ef8",fontWeight:700,textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Live Train Status</div>
          {trainStatus ? (
            <div style={{fontSize:12,color:"#f0f0f8",lineHeight:1.7}}>
              <div>Train {trainStatus.train} · {trainStatus.state}</div>
              <div>Delay: {trainStatus.delay}</div>
              <div>Next stop ETA: {trainStatus.nextStopIn}</div>
              <div>Pickup station: {trainStatus.station}</div>
            </div>
          ) : (
            <div style={{fontSize:12,color:"#8888aa"}}>Train status appears after search.</div>
          )}
        </div>

        <div style={{marginTop:10,background:"#1c1c28",border:"1px solid #2a2a3a",borderRadius:12,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:"#00e5a0",fontWeight:700,textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Cab Request Status</div>
          <div style={{fontSize:12,color:"#f0f0f8"}}>{requestStatus}</div>
        </div>

        <div style={{marginTop:10,background:"#1c1c28",border:"1px solid #2a2a3a",borderRadius:12,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:"#f59e0b",fontWeight:700,textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Passenger Notification</div>
          <div style={{fontSize:12,color:"#f0f0f8"}}>{passengerUpdate}</div>
        </div>
      </div>
